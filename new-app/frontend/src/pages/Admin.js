...
const Admin = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        try {
          ...
          // In caso di errore, usa le configurazioni di default
          setConfigs({
            paypal_enabled: { state: false, value: '' },
            home_notes_enabled: { state: false, value: '' },
            external_menu_enabled: { state: false, value: '' },
        ...
        // Assicurati che menuOptions sia sempre un array
  ...
  const handleAddMenuItem = async () => {
    try {
      ...
      // Ricarica il menu dopo l'aggiunta
      const menuResponse = await api.menu.getAll();
      setMenuOptions(menuResponse.data.data.options || []);
  ...
  const handleDeleteMenuItem = async (id) => {
    try {
      ...
      setError('Si è verificato un errore durante l\'eliminazione dell\'opzione menu.');
    }
  };
  ...
  const handleCopyOrdersToClipboard = () => {
    // Crea un testo formattato con il riepilogo degli ordini nel formato richiesto
    let clipboardText = "";
    ...
    navigator.clipboard.writeText(clipboardText)
      ...
      .catch(err => {
        ...
        alert('Impossibile copiare negli appunti. Verifica i permessi del browser.');
      });
  };
  ...
  const handleSavePayment = async () => {
    try {
      ...
      const displayName = orderSummary.users.find(u => u.username === selectedUser.username)?.displayName || selectedUser.username;

      // Usa il servizio paymentService corretto
      const response = await api.payment.create({
        username: selectedUser.username,
        display_name: displayName,
        amount_paid: parseFloat(amountPaid),
        total_amount: totalAmount,
        is_completed: paymentComplete[selectedUser.username] || false
      });
      ...
      setPayments(prev => ({
        ...
        [selectedUser.username]: {
          ...
          completed: paymentComplete[selectedUser.username] || false
        }
      }));
  ...
  const handleResetMenu = async () => {
    if (window.confirm('Sei sicuro di voler azzerare il menu? Tutti gli ordini e i pagamenti verranno eliminati.')) {
      try {
        ...
        alert('Menu azzerato con successo!');
      } catch (error) {
        console.error('Errore durante l\'azzeramento del menu:', error);
        alert('Si è verificato un errore durante l\'azzeramento del menu.');
      }
    }
  };
  ...
  const handleCopyAllOrdersToClipboard = () => {
    try {
      const users = orderSummary.users;
      let summaryText = "RIEPILOGO ORDINI\n\n";
  ...
  const handleExportHistoryToCSV = () => {
    try {
      ...
      alert('Si è verificato un errore durante l\'esportazione dello storico.');
    }
  };
                          ...
                          {payments[userSummary.username] ? (
                            <div>
                              ...
                              <small>
                                ...
                                Resto: €{parseFloat(payments[userSummary.username].change).toFixed(2)}
                                {payments[userSummary.username].change > 0 && (
                                  <>
                                    ...
                                    <br />
                                    Resto da dare: €{parseFloat(payments[userSummary.username].change - (payments[userSummary.username].partial_change_given || 0)).toFixed(2)}
                                  </>
                                )}
                              </small>
                              <div className="mt-2">
                ...
                <Card>
                  ...
                  <Card.Body>
                    <div className="d-flex align-items-center justify-content-between mb-3">
                      <div>
                        <h6 className="mb-0">Abilita Menu Esterno</h6>
                        <p className="text-muted mb-0">Permetti agli utenti di visualizzare un menu esterno</p>
                      </div>
                      <Button 
                        variant={externalMenuEnabled ? "danger" : "success"}
                        onClick={() => handleToggleExternalMenu(!externalMenuEnabled)}
                      >
                        {externalMenuEnabled ? "Disabilita Menu Esterno" : "Abilita Menu Esterno"}
                      </Button>
                    </div>
                    ...
                    {externalMenuEnabled && (
                      <div className="mt-4">
                        ...
                        <p className="text-muted mt-2 mb-0">
                          Il link verrà mostrato agli utenti nella pagina del menu
                        </p>
                      </div>
                    )}
                  </Card.Body>
                ...
                <Card>
                  ...
                  <Card.Body>
                    <Form.Group className="mb-3">
                      ...
                      <Form.Check
                        type="switch"
                        label="Abilita pagamenti PayPal"
            ...
            <Card>
              ...
              <Card.Body>
                {menuOptions && menuOptions.length > 0 ? (
                  ...
                  <Alert variant="info">Nessuna opzione menu disponibile per la data odierna.</Alert>
                )}
              </Card.Body>
            </Card>
          </div>
                  ...
                  <Card className="mb-4">
                    ...
                    <Card.Body>
                      <div className="d-grid gap-2">
                        <Button variant="outline-primary" onClick={handleCopyAllOrdersToClipboard}>
                          <FontAwesomeIcon icon={faClipboard} className="me-2" />
                          Copia Riepilogo
                        </Button>
                        <Button variant="outline-success" onClick={handleMarkAllPaid}>
                          <FontAwesomeIcon icon={faCheckDouble} className="me-2" />
                          Marca Tutti Pagati
                        </Button>
                        <Button variant="outline-danger" onClick={handleClearAllPayments}>
                  ...
                  <Card.Body>
                    <div className="d-flex justify-content-between align-items-center mb-3">
                      <div>
                        ...
                        <p className="text-muted mb-0">Abilita i pagamenti tramite PayPal.me</p>
                      </div>
                      <Form.Check
                        type="switch"
                        checked={configs.paypal_enabled.state}
                        onChange={(e) => handleUpdateConfig('paypal_enabled', e.target.checked)}
                      />
                    </div>
                    {configs.paypal_enabled.state && (
                      <Form.Group className="mt-3">
                        ...
                        <Form.Control
                          type="text"
                          value={configs.paypal_enabled.value}
                          onChange={(e) => handleUpdateConfig('paypal_enabled', true, e.target.value)}
                          placeholder="Inserisci il tuo username PayPal.me"
                        />
                      </Form.Group>
                    )}
                  ...
                  <Card.Body>
                    <div className="d-flex justify-content-between align-items-center mb-3">
                      ...
                      <Form.Check
                        ...
                        onChange={(e) => handleUpdateConfig('home_notes_enabled', e.target.checked)}
                      />
                    </div>
                    {configs.home_notes_enabled.state && (
                      <Form.Group className="mt-3">
                        ...
                        <Form.Control
                          as="textarea"
                          rows={4}
                          value={configs.home_notes_enabled.value}
                          onChange={(e) => handleUpdateConfig('home_notes_enabled', true, e.target.value)}
                          placeholder="Inserisci le note da mostrare nella home"
                        />
                      </Form.Group>
                    )}
              ...
              <Col md={6}>
                <Card className="mb-4">
                  ...
                  <Card.Body>
                    <div className="d-flex justify-content-between align-items-center mb-3">
                      <div>
                        <h6 className="mb-0">Menu Esterno</h6>
                        <p className="text-muted mb-0">Abilita la visualizzazione di un menu esterno</p>
                      </div>
                      <Form.Check
                        type="switch"
                        checked={configs.external_menu_enabled.state}
                        onChange={(e) => handleUpdateConfig('external_menu_enabled', e.target.checked)}
                      />
                    </div>
                    {configs.external_menu_enabled.state && (
                      <Form.Group className="mt-3">
                        ...
                        <Form.Control
                          type="url"
                          value={configs.external_menu_enabled.value}
                          onChange={(e) => handleUpdateConfig('external_menu_enabled', true, e.target.value)}
                          placeholder="Inserisci il link al menu esterno"
                        />
                      </Form.Group>
                    )}
