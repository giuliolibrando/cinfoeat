...
const Home = () => {
  ...
  // Funzione per validare e formattare l'URL esterno
  ...
  const updatePaymentInfo = async (total) => {
    try {
      ...
      setPaymentInfo({
        total: total,
        paid: paymentData.amount_paid || 0,
        remaining: paymentData.change_amount || total,
        partial_change_given: paymentData.partial_change_given || 0
      });
    } catch (paymentError) {
  ...
  useEffect(() => {
    ...
    if (userChoices.total > 0) {
      ...
      // Imposta l'aggiornamento ogni 5 secondi
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log('Richiedo configurazione menu esterno...');
        const externalMenuResponse = await publicService.getExternalMenuConfig();
        console.log('Risposta menu esterno:', externalMenuResponse.data);
        setExternalMenuConfig(externalMenuResponse.data.data);

        // Carica le note della home
        console.log('Richiedo note home...');
        const homeNotesResponse = await publicService.getHomeNotes();
        console.log('Risposta note home:', homeNotesResponse.data);
        ...
        try {
          ...
          setPaypalEmail(paypalResponse.data.data.value);
        } catch (paypalError) {
          console.error('Error loading PayPal configuration:', paypalError);
          setPaypalConfig({ state: false, value: '' });
        }
  ...
  const checkOrderState = async () => {
    try {
      // Prima tenta di usare l'endpoint pubblico aperto a tutti
      const publicOrderStateResponse = await publicService.getOrderState();
      console.log('Order state response from public endpoint:', publicOrderStateResponse);
      setOrderState(publicOrderStateResponse.data.data.state);
    } catch (publicEndpointError) {
      console.error('Error getting order state from public endpoint:', publicEndpointError);
      
      // Se fallisce, tenta con l'endpoint choices
      try {
        console.log('Attempting to get order state via userChoiceService');
        const orderStateResponse = await userChoiceService.getOrderState();
        console.log('Order state response from user choice endpoint:', orderStateResponse);
        setOrderState(orderStateResponse.data.data.state);
      } catch (choicesEndpointError) {
        console.error('Error getting order state from choices endpoint:', choicesEndpointError);
        
        // Se fallisce, tenta con l'endpoint admin (se l'utente è admin)
        try {
          if (user?.isAdmin) {
            console.log('Attempting to get order state via admin endpoint');
            const adminOrderStateResponse = await adminService.getConfig('order_state');
            console.log('Order state response from admin endpoint:', adminOrderStateResponse);
            setOrderState(adminOrderStateResponse.data.data.state);
          } else {
            // Per gli utenti non admin, impostiamo di default gli ordini come aperti
            console.log('Setting default order state to open for non-admin users');
            setOrderState(true);
          }
        } catch (adminEndpointError) {
          console.error('Error getting order state from admin endpoint:', adminEndpointError);
          // Fallback: impostiamo gli ordini come aperti (invece che chiusi)
          console.log('Setting fallback order state to open');
          setOrderState(true);
        }
      }
    }
  ...
  const renderPaymentInfo = () => {
    ...
    const remainingChange = remaining - partialChangeGiven;
        ...
        <Card.Body>
          ...
          {remaining > 0 && (
            <>
              ...
              {remainingChange <= 0 && partialChangeGiven > 0 && (
                <div className="d-flex justify-content-between align-items-center mb-3">
                  <span className="fw-bold">Resto da ricevere:</span>
                  <span className="fs-5 text-warning">€{Math.abs(remainingChange).toFixed(2)}</span>
                </div>
              )}
            ...
            <>
              ...
              {partialChangeGiven > 0 && (
                <div className="d-flex justify-content-between align-items-center">
                  <span className="fw-bold">Resto da ricevere:</span>
                  <span className="fs-5 text-warning">€{partialChangeGiven.toFixed(2)}</span>
                </div>
              )}
            </>
          )}
  ...
  return (
    <Container fluid className="py-4">
      ...
      {externalMenuConfig.state && externalMenuConfig.value !== '' && (
